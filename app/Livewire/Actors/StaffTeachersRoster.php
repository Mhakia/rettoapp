...
#[Title('Profesores')]
class StaffTeachersRoster extends Component
{
    use WithPagination;
    ...
    /**
     * Keyset (cursor) pagination stays fast no matter how many teachers exist across institutions.
     */
    #[Computed]
    public function memberships()
    {
        ...
        return InstitutionMembership::with(['institution', 'user.teacherGroups'])
            ->where('status', 'active')
            ...
            ->when($this->groupId, function ($query) {
                $query->whereHas('user.teacherGroups', fn ($g) => $g->where('groups.id', $this->groupId));
            })
    ...
    {
        return view('livewire.actors.staff-teachers-roster');
    }
}
