<section class="w-full">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-teal-border bg-teal-bg px-6 py-5">
        <div>
            ...
            <flux:text class="text-brand-text-muted!">{{ __('Directorio de profesores de todas las instituciones.') }}</flux:text>
        </div>
        @if ($this->selectedInstitution)
            @can('manageActors', $this->selectedInstitution)
                <div class="flex flex-wrap items-center gap-2">
                    <flux:button icon="user-plus" variant="primary" href="{{ route('actors.teachers.create', ['institution' => $this->selectedInstitution->uuid]) }}" wire:navigate>{{ __('Nuevo profesor') }}</flux:button>
                    <flux:button icon="arrow-up-tray" href="{{ route('actors.teachers.import', ['institution' => $this->selectedInstitution->uuid]) }}" wire:navigate>{{ __('Cargar profesores') }}</flux:button>
                </div>
            @endcan
        @endif
    </div>
    ...
    <div
        wire:key="staff-teachers-{{ $this->membershipsCacheKey }}"
        x-data="{
            selected: null,
        ...
        <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    ...
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <flux:text class="block truncate text-sm text-brand-text-muted!">
                                {{ $membership->institution->name }} · {{ $membership->user->document_number ?? __('Sin documento') }}
                            </flux:text>
                        </div>
                    </div>
            ...
            @empty
                <div class="flex flex-col items-center gap-3 px-6 py-16 text-center">
                    ...
                    <flux:text class="text-brand-text-muted!">{{ __('No se encontraron profesores con estos filtros.') }}</flux:text>
                </div>
            @endforelse
        </div>
