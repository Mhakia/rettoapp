<section class="w-full">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-teal-border bg-teal-bg px-6 py-5">
        <div>
            ...
            <flux:text class="text-brand-text-muted!">{{ __('Directorio de estudiantes de todas las instituciones.') }}</flux:text>
        </div>
        @if ($this->selectedInstitution)
            @can('manageActors', $this->selectedInstitution)
                <div class="flex flex-wrap items-center gap-2">
                    <flux:button icon="academic-cap" variant="primary" href="{{ route('actors.students.create', ['institution' => $this->selectedInstitution->uuid]) }}" wire:navigate>{{ __('Nuevo estudiante') }}</flux:button>
                    <flux:button icon="arrow-up-tray" href="{{ route('actors.students.import', ['institution' => $this->selectedInstitution->uuid]) }}" wire:navigate>{{ __('Cargar estudiantes') }}</flux:button>
                </div>
            @endcan
        @endif
    </div>
    ...
    <div
        wire:key="staff-students-{{ $this->membershipsCacheKey }}"
        x-data="{
            selected: null,
        ...
        <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
                ...
                <div wire:key="membership-{{ $membership->id }}" class="flex flex-wrap items-center justify-between gap-4 px-5 py-4 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <flux:text class="block truncate text-sm text-brand-text-muted!">
                                {{ $membership->institution->name }} · {{ $membership->user->studentProfile?->document_number ?? __('Sin documento') }}
                            </flux:text>
                        </div>
                    </div>
                    <div class="flex flex-wrap items-center justify-end gap-2">
                        ...
                        <div class="ml-1 flex items-center gap-1 border-l border-zinc-200 pl-2 dark:border-zinc-700">
                            ...
                            @can('manageActors', $membership->institution)
            ...
            @empty
                <div class="flex flex-col items-center gap-3 px-6 py-16 text-center">
                    ...
                    <flux:text class="text-brand-text-muted!">{{ __('No se encontraron estudiantes con estos filtros.') }}</flux:text>
                </div>
            @endforelse
        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3 rounded-lg bg-zinc-50 p-4 text-sm dark:bg-zinc-800/50">
                        <div class="col-span-2">
                            <div class="text-xs font-semibold text-brand-text-muted uppercase">{{ __('Institución') }}</div>
                            <div class="font-medium text-brand-text" x-text="selected.institution"></div>
                        </div>
                        <div>
