<section class="w-full">
    <div class="mb-8 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-zinc-200 bg-white px-6 py-5">
        <div>
            <flux:heading size="xl">{{ __('billing_invoices_title') }}</flux:heading>
            <flux:text class="text-brand-text-muted!">{{ __('billing_invoices_description') }}</flux:text>
        </div>
    </div>

    <div class="mb-6 flex flex-wrap gap-3">
        <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass" :placeholder="__('billing_invoices_search_placeholder')" class="max-w-md" />
    ...
    <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
        <div class="divide-y divide-zinc-100">
            ...
            @empty
                <flux:text class="block px-6 py-10 text-center text-brand-text-muted!">{{ __('billing_invoices_no_results') }}</flux:text>
            @endforelse
        </div>
    </div>
